...
jest.mock('../../../presentation/traces/TraceModal', () => ({
  ...
  default: ({
    ...
    return (
      <div data-testid="trace-modal-overlay">
        <div data-testid="trace-modal-content">
          <button
            onClick={() =>
              onAdd({
                dateSale: '2024-01-01',
...
describe('PropertyModal', () => {
  ...
  it('abre TraceModal y agrega un trace', async () => {
    ...
    expect(screen.getByTestId('trace-modal-content')).toBeInTheDocument();

    // Click en agregar trace
    const addTraceButton = screen.getByTestId('add-trace-button');
    await userEvent.click(addTraceButton);

    // Verificamos que el trace se agregó (aquí solo comprobamos que siga visible el modal)
    expect(screen.getByTestId('trace-modal-content')).toBeInTheDocument();
  });
});
